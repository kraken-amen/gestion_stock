...
export default function Otp() {
  ...
  return (
    <div className="min-h-screen relative font-sans text-white overflow-x-hidden">
      {/* Background */}
      <div className="fixed inset-0 bg-gradient-to-br from-slate-950 via-blue-950 to-purple-900"></div>
        ...
        <div className="w-full max-w-md">
          ...
          <div className="backdrop-blur-xl bg-white/10 rounded-2xl md:rounded-3xl p-6 md:p-10 border border-white/20 shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-6 md:space-y-8">
              {/* OTP Inputs: grid for precise mobile layout */}
              <div className="grid grid-cols-6 gap-2 md:gap-3">
                {otp.map((digit, index) => (
                  <input
                    ...
                    // Classes Responsive: w-full pour s'adapter à la grille
                    className="w-full aspect-square text-center text-xl md:text-2xl font-bold bg-white/5 border border-white/10 focus:border-blue-500 rounded-lg md:rounded-xl outline-none transition-all"
                  />
                ))}
              </div>
              ...
              <div className="flex justify-end">
                ...
                >
                  <Send size={10} className="md:w-[12px]" /> {isResending ? "Renvoyer..." : "Renvoyer maintenant"}
                </button>
              </div>
              ...
              <button
                ...
                className="w-full py-3 md:py-4 rounded-xl font-bold bg-blue-600 hover:bg-blue-500 shadow-lg shadow-blue-900/20 transition-all flex items-center justify-center gap-2 disabled:opacity-50 text-sm md:text-base"
              >
            ...
            <div className="mt-6 md:mt-8 text-center">
              {timer > 0 ? (
                ...
                <button onClick={handleResend} className="text-blue-400 hover:underline flex items-center gap-2 mx-auto text-xs md:text-sm">
                  <RefreshCcw size={14} /> Demander un nouveau code
                </button>
              )}
            </div>
          </div>
      ...
      {/* Toasts: Positioning on mobile */}
      <div className="fixed top-4 md:top-6 right-4 md:right-6 left-4 md:left-auto z-50 space-y-3">
        {toasts.map((toast) => (
