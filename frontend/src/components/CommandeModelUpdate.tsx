...
const CommandeModelUpdate = ({ isOpen, onClose, onCommandeUpdated, commande }: PropsCommandeUpdate) => {
    ...
    return (
        <div className="font-sans fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-lg bg-gradient-to-br from-slate-950 via-slate-900 to-amber-950/30 border border-white/20 rounded-2xl p-8 shadow-2xl overflow-hidden max-h-[95vh] flex flex-col">
                ...
                <div className="flex justify-between items-center mb-6 relative z-10">
                    ...
                    <button
                        type="button"
                        onClick={addItem}
                        className="flex items-center gap-2 text-xs bg-amber-600/20 hover:bg-amber-600/40 text-amber-400 border border-amber-500/30 px-3 py-1.5 rounded-lg transition-all active:scale-95"
                    >
                ...
                <form onSubmit={handleSubmit} className="space-y-6 relative z-10 overflow-y-auto pr-2 custom-scrollbar">
                            ...
                            <div key={index} className="relative p-4 bg-white/5 border border-white/10 rounded-xl space-y-3">
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        placeholder="Ex: 100"
                                        className="w-full bg-white/5 border-2 border-white/10 rounded-xl px-4 py-2.5 text-white focus:border-amber-500/50 focus:outline-none transition-all text-sm font-medium"
                                    />
                                </div>
                            </div>
                        ))}
                    ...
                    <div className="flex gap-3 pt-4 border-t border-white/10 sticky bottom-0 bg-slate-900/10 backdrop-blur-md pb-1">
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="flex-1 py-3 rounded-xl font-bold text-white bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-700 hover:to-orange-700 transition-all shadow-lg shadow-amber-900/20 flex items-center justify-center gap-2 disabled:opacity-50"
                        >
                            {loading ? (
